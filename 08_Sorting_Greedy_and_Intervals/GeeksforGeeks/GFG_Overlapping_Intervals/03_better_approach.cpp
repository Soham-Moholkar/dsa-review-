...
This particular file uses the "Sort and merge into answer" approach.
...
----------------------------
Approach name: Sort and merge into answer
...
11. `if(arr[i][0] <= answer.back()[1]) {`
   Runs the next block only when `arr[i][0] <= answer.back()[1]` is true.
12. `answer.back()[1] = max(answer.back()[1], arr[i][1]);`
   Updates `answer.back()[1]` to `max(answer.back()[1], arr[i][1])` for the next step of the algorithm.
13. `else {`
   Handles the remaining case after the preceding condition(s) were false.
14. `answer.push_back(arr[i]);`
   Appends the computed value to the end of the result/container.
15. `return answer;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Sort and merge into answer" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Sort and merge into answer" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
