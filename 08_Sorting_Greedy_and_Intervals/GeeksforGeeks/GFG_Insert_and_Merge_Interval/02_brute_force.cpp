...
This particular file uses the "Append, sort, then merge all" approach.
...
----------------------------
Approach name: Append, sort, then merge all
...
10. `if(answer.empty() || intervals[i][0] > answer.back()[1]) {`
   Runs the next block only when `answer.empty() || intervals[i][0] > answer.back()[1]` is true.
11. `answer.push_back(intervals[i]);`
   Appends the computed value to the end of the result/container.
12. `else {`
   Handles the remaining case after the preceding condition(s) were false.
13. `answer.back()[1] = max(answer.back()[1], intervals[i][1]);`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Append, sort, then merge all" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Append, sort, then merge all" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
