...
public:
    vector<vector<int>> insert(vector<vector<int>>& intervals, vector<int>& newInterval) {
        ...
        int i = 0;
        while(i < intervals.size() && intervals[i][1] < newInterval[0]) answer.push_back(intervals[i++]);
...
This particular file uses the "Three-phase scan" approach.
...
- `insert` is the function name the online judge calls.
...
----------------------------
Approach name: Three-phase scan
...
5. `vector<vector<int>> insert(vector<vector<int>>& intervals, vector<int>& newInterval) {`
...
8. `while(i < intervals.size() && intervals[i][1] < newInterval[0]) answer.push_back(intervals[i++]);`
   Repeats the following block while `i < intervals.size() && intervals[i][1] < newInterval[0]` is true. Its one-line body is `answer.push_back(intervals[i++]);`.
9. `while(i < intervals.size() && intervals[i][0] <= newInterval[1]) {`
...
13. `answer.push_back(newInterval);`
   Appends the computed value to the end of the result/container.
14. `while(i < intervals.size()) answer.push_back(intervals[i++]);`
   Repeats the following block while `i < intervals.size()` is true. Its one-line body is `answer.push_back(intervals[i++]);`.
15. `return answer;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Three-phase scan" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Three-phase scan" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
