...
This particular file uses the "One sorted pass" approach.
...
7. `if(intervals.size() == 0) return answer;`
   Runs the next block only when `intervals.size() == 0` is true. The one-line action is `return answer;`.
...
9. `answer.push_back(intervals[0]);`
...
11. `if(intervals[i][0] <= answer.back()[1]) {`
   Runs the next block only when `intervals[i][0] <= answer.back()[1]` is true.
12. `answer.back()[1] = max(answer.back()[1], intervals[i][1]);`
...
14. `answer.push_back(intervals[i]);`
   Appends the computed value to the end of the result/container.
15. `return answer;`
