...
This particular file uses the "Sort and merge" approach.
...
6. `if(intervals.empty()) return {};`
   Runs the next block only when `intervals.empty()` is true. The one-line action is `return {};`.
...
8. `vector<vector<int>> answer = {intervals[0]};`
   Creates `answer` and initializes it from `{intervals[0]}`. This gives the algorithm its starting state.
9. `for(int i = 1; i < intervals.size(); i++) {`
   Starts a loop: first `int i = 1`; keep repeating while `i < intervals.size()` is true; after each iteration perform `i++`.
10. `if(intervals[i][0] <= answer.back()[1]) {`
   Runs the next block only when `intervals[i][0] <= answer.back()[1]` is true.
11. `answer.back()[1] = max(answer.back()[1], intervals[i][1]);`
   Updates `answer.back()[1]` to `max(answer.back()[1], intervals[i][1])` for the next step of the algorithm.
12. `else answer.push_back(intervals[i]);`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Sort and merge" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Sort and merge" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
