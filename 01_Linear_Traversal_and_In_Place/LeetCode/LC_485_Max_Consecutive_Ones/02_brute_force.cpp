...
This particular file uses the "Start a streak from every index" approach.
...
----------------------------
Approach name: Start a streak from every index
...
9. `for(int j = i; j < nums.size(); j++) {`
   Starts a loop: first `int j = i`; keep repeating while `j < nums.size()` is true; after each iteration perform `j++`.
10. `if(nums[j] == 1) {`
   Runs the next block only when `nums[j] == 1` is true.
11. `currentCount++;`
   Moves the relevant counter or pointer by one position.
12. `maximumCount = max(maximumCount, currentCount);`
