...
This particular file uses the "Digit count with division and no extra storage" approach.
...
----------------------------
Approach name: Digit count with division and no extra storage
...
8. `int number = nums[i];`
   Creates `number` and initializes it from `nums[i]`. This gives the algorithm its starting state.
9. `int digits = 0;`
