...
This particular file uses the "Convert every number to text" approach.
...
----------------------------
Approach name: Convert every number to text
...
8. `string value = to_string(abs(nums[i]));`
   Updates `string value` to `to_string(abs(nums[i]))` for the next step of the algorithm.
9. `if(value.size() % 2 == 0) {`
   Runs the next block only when `value.size() % 2 == 0` is true.
10. `answer++;`
...
- Extra-space complexity: O(max digits).
