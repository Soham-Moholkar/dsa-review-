...
This particular file uses the "Use an additional result vector" approach.
...
----------------------------
Approach name: Use an additional result vector
...
10. `while(answer.size() < nums.size()) {`
   Repeats the following block while `answer.size() < nums.size()` is true.
