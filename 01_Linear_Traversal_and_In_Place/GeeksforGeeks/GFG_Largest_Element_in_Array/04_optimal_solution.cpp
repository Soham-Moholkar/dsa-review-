...
This particular file uses the "Single linear scan" approach.
...
----------------------------
Approach name: Single linear scan
...
6. `int largestValue = arr[0];`
   Creates `largestValue` and initializes it from `arr[0]`. This gives the algorithm its starting state.
...
8. `if(arr[i] > largestValue) {`
   Runs the next block only when `arr[i] > largestValue` is true.
9. `largestValue = arr[i];`
   Updates `largestValue` to `arr[i]` for the next step of the algorithm.
10. `return largestValue;`
   Ends the function and sends `largestValue` back to the caller.
