...
This particular file uses the "Compare every candidate" approach.
...
----------------------------
Approach name: Compare every candidate
...
7. `bool isLargest = true;`
   Creates `isLargest` and initializes it from `true`. This gives the algorithm its starting state.
...
10. `isLargest = false;`
   Updates `isLargest` to `false` for the next step of the algorithm.
11. `break;`
   Stops the nearest loop immediately because no more iterations are needed.
12. `if(isLargest) {`
   Runs the next block only when `isLargest` is true.
13. `return arr[i];`
