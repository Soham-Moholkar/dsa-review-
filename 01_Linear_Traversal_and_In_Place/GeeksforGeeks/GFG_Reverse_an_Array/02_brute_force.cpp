...
This particular file uses the "Build a reversed copy" approach.
...
----------------------------
Approach name: Build a reversed copy
...
7. `for(int i = arr.size() - 1; i >= 0; i--) {`
   Starts a loop: first `int i = arr.size() - 1`; keep repeating while `i >= 0` is true; after each iteration perform `i--`.
