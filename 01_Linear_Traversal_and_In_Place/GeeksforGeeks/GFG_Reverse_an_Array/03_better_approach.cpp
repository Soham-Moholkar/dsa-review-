...
This particular file uses the "Swap using indexed half traversal" approach.
...
- `reverseArray` is the function name the online judge calls.
...
----------------------------
Approach name: Swap using indexed half traversal
...
8. `int temp = arr[i];`
   Creates `temp` and initializes it from `arr[i]`. This gives the algorithm its starting state.
9. `arr[i] = arr[n - i - 1];`
   Updates `arr[i]` to `arr[n - i - 1]` for the next step of the algorithm.
10. `arr[n - i - 1] = temp;`
   Updates `arr[n - i - 1]` to `temp` for the next step of the algorithm.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Swap using indexed half traversal" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Swap using indexed half traversal" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
