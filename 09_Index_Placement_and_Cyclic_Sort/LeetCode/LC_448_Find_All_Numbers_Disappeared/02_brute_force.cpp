...
This particular file uses the "Search for every value" approach.
...
----------------------------
Approach name: Search for every value
...
7. `for(int value = 1; value <= nums.size(); value++) {`
...
10. `if(nums[i] == value) found = true;`
   Runs the next block only when `nums[i] == value` is true. The one-line action is `found = true;`.
11. `if(!found) answer.push_back(value);`
   Runs the next block only when `!found` is true. The one-line action is `answer.push_back(value);`.
12. `return answer;`
