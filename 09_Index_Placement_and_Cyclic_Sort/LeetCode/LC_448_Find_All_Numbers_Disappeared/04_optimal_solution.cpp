...
7. `int index = abs(nums[i]) - 1;`
   Creates `index` and initializes it from `abs(nums[i]) - 1`. This gives the algorithm its starting state.
8. `if(nums[index] > 0) nums[index] = -nums[index];`
   Runs the next block only when `nums[index] > 0` is true. The one-line action is `nums[index] = -nums[index];`.
9. `vector<int> answer;`
   Declares `answer` so it can store state used by the algorithm.
10. `for(int i = 0; i < nums.size(); i++) {`
   Starts a loop: first `int i = 0`; keep repeating while `i < nums.size()` is true; after each iteration perform `i++`.
11. `if(nums[i] > 0) answer.push_back(i + 1);`
   Runs the next block only when `nums[i] > 0` is true. The one-line action is `answer.push_back(i + 1);`.
12. `return answer;`
...
-------------
- Time complexity: O(n).
- Extra-space complexity: O(1) extra.
