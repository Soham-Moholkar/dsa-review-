...
This particular file uses the "Test every candidate" approach.
...
----------------------------
Approach name: Test every candidate
...
6. `for(int value = 0; value <= nums.size(); value++) {`
   Starts a loop: first `int value = 0`; keep repeating while `value <= nums.size()` is true; after each iteration perform `value++`.
7. `bool found = false;`
...
9. `if(nums[i] == value) found = true;`
   Runs the next block only when `nums[i] == value` is true. The one-line action is `found = true;`.
10. `if(!found) return value;`
   Runs the next block only when `!found` is true. The one-line action is `return value;`.
11. `return -1;`
