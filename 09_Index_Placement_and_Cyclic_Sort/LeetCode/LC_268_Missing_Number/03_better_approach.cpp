...
This particular file uses the "Sort and compare index to value" approach.
...
----------------------------
Approach name: Sort and compare index to value
...
8. `if(nums[i] != i) return i;`
   Runs the next block only when `nums[i] != i` is true. The one-line action is `return i;`.
9. `return nums.size();`
   Ends the function and sends `nums.size()` back to the caller.
