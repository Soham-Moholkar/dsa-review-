...
This particular file uses the "XOR indexes and values" approach.
...
----------------------------
Approach name: XOR indexes and values
...
6. `int answer = nums.size();`
   Creates `answer` and initializes it from `nums.size()`. This gives the algorithm its starting state.
7. `for(int i = 0; i < nums.size(); i++) {`
   Starts a loop: first `int i = 0`; keep repeating while `i < nums.size()` is true; after each iteration perform `i++`.
8. `answer = answer ^ i;`
   Updates `answer` to `answer ^ i` for the next step of the algorithm.
9. `answer = answer ^ nums[i];`
   Updates `answer` to `answer ^ nums[i]` for the next step of the algorithm.
10. `return answer;`
