...
7. `int required = 1;`
   Creates `required` and initializes it from `1`. This gives the algorithm its starting state.
8. `for(int i = 0; i < nums.size(); i++) {`
   Starts a loop: first `int i = 0`; keep repeating while `i < nums.size()` is true; after each iteration perform `i++`.
9. `if(nums[i] == required) required++;`
   Runs the next block only when `nums[i] == required` is true. The one-line action is `required++;`.
10. `return required;`
   Ends the function and sends `required` back to the caller.
