...
This particular file uses the "Cyclic placement" approach.
...
----------------------------
Approach name: Cyclic placement
...
9. `if(arr[i] >= 1 && arr[i] <= n && arr[i] != arr[arr[i] - 1]) {`
   Runs the next block only when `arr[i] >= 1 && arr[i] <= n && arr[i] != arr[arr[i] - 1]` is true.
10. `swap(arr[i], arr[arr[i] - 1]);`
