...
This particular file uses the "Count every value separately" approach.
...
----------------------------
Approach name: Count every value separately
...
8. `for(int value = 0; value < n; value++) {`
...
11. `if(arr[i] == value) count++;`
   Runs the next block only when `arr[i] == value` is true. The one-line action is `count++;`.
12. `if(count > 1) answer.push_back(value);`
   Runs the next block only when `count > 1` is true. The one-line action is `answer.push_back(value);`.
13. `if(answer.empty()) answer.push_back(-1);`
