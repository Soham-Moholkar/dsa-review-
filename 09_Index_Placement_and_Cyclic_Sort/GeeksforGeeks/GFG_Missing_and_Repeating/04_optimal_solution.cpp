...
This particular file uses the "Sign marking" approach.
...
9. `int value = abs(arr[i]);`
   Creates `value` and initializes it from `abs(arr[i])`. This gives the algorithm its starting state.
10. `int index = value - 1;`
   Creates `index` and initializes it from `value - 1`. This gives the algorithm its starting state.
11. `if(arr[index] < 0) {`
   Runs the next block only when `arr[index] < 0` is true.
12. `repeating = value;`
   Updates `repeating` to `value` for the next step of the algorithm.
13. `else {`
   Handles the remaining case after the preceding condition(s) were false.
14. `arr[index] = -arr[index];`
   Updates `arr[index]` to `-arr[index]` for the next step of the algorithm.
15. `int missing = -1;`
...
18. `missing = i + 1;`
   Updates `missing` to `i + 1` for the next step of the algorithm.
19. `break;`
   Stops the nearest loop immediately because no more iterations are needed.
20. `return {repeating, missing};`
...
A negative value at an index means its corresponding number has already appeared.
