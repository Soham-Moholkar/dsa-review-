...
This particular file uses the "Count every possible value" approach.
...
- `findTwoElement` is the function name the online judge calls.
...
----------------------------
Approach name: Count every possible value
...
5. `vector<int> findTwoElement(vector<int>& arr) {`
   Defines the judge-facing function and lists the inputs it receives.
6. `int repeating = -1;`
   Creates `repeating` and initializes it from `-1`. This gives the algorithm its starting state.
7. `int missing = -1;`
   Creates `missing` and initializes it from `-1`. This gives the algorithm its starting state.
8. `int n = arr.size();`
...
12. `if(arr[i] == value) count++;`
   Runs the next block only when `arr[i] == value` is true. The one-line action is `count++;`.
13. `if(count == 0) missing = value;`
   Runs the next block only when `count == 0` is true. The one-line action is `missing = value;`.
14. `if(count == 2) repeating = value;`
   Runs the next block only when `count == 2` is true. The one-line action is `repeating = value;`.
15. `return {repeating, missing};`
   Ends the function and sends `{repeating, missing}` back to the caller.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Count every possible value" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Count every possible value" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
