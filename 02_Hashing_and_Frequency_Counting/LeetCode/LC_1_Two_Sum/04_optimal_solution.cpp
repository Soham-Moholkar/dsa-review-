...
This particular file uses the "One-pass hashmap" approach.
...
----------------------------
Approach name: One-pass hashmap
...
8. `long long remaining = 1LL * target - nums[i];`
...
9. `if(mp.find(remaining) != mp.end()) {`
   Runs the next block only when `mp.find(remaining) != mp.end()` is true.
10. `return {mp[remaining], i};`
   Ends the function and sends `{mp[remaining], i}` back to the caller.
11. `mp[nums[i]] = i;`
