...
6. `if(nums.size() == 0) {`
   Runs the next block only when `nums.size() == 0` is true.
7. `return 0;`
...
14. `if(nums[i] == nums[i - 1] + 1) {`
   Runs the next block only when `nums[i] == nums[i - 1] + 1` is true.
15. `currentLength++;`
...
18. `answer = max(answer, currentLength);`
   Updates `answer` to `max(answer, currentLength)` for the next step of the algorithm.
19. `return answer;`
