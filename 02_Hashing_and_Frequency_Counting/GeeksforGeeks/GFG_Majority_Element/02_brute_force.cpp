...
This particular file uses the "Count each candidate" approach.
...
----------------------------
Approach name: Count each candidate
...
9. `if(arr[j] == arr[i]) {`
   Runs the next block only when `arr[j] == arr[i]` is true.
10. `count++;`
   Moves the relevant counter or pointer by one position.
11. `if(count > arr.size() / 2) {`
   Runs the next block only when `count > arr.size() / 2` is true.
12. `return arr[i];`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Count each candidate" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Count each candidate" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
