...
This particular file uses the "Boyer-Moore plus verification" approach.
...
----------------------------
Approach name: Boyer-Moore plus verification
...
6. `int candidate = 0;`
   Creates `candidate` and initializes it from `0`. This gives the algorithm its starting state.
7. `int count = 0;`
...
10. `candidate = arr[i];`
   Updates `candidate` to `arr[i]` for the next step of the algorithm.
11. `count = 1;`
   Updates `count` to `1` for the next step of the algorithm.
12. `else if(arr[i] == candidate) {`
   Defines the judge-facing function and lists the inputs it receives.
13. `count++;`
...
17. `for(int i = 0; i < arr.size(); i++) {`
   Starts a loop: first `int i = 0`; keep repeating while `i < arr.size()` is true; after each iteration perform `i++`.
18. `if(arr[i] == candidate) {`
   Runs the next block only when `arr[i] == candidate` is true.
19. `count++;`
   Moves the relevant counter or pointer by one position.
20. `if(count > arr.size() / 2) {`
