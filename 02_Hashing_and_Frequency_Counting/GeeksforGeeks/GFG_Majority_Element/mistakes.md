# Mistake Analysis

## Original-attempt status

No exact prior attempt was available, so no mistake has been invented or attributed to you.

## Common mistakes for this problem

1. Returning the Boyer-Moore candidate without verification is wrong when a majority is not guaranteed.
2. The threshold is strictly greater than n/2, not greater than or equal.
3. Reset the candidate only when count becomes zero.

## Your exact mistake log

After solving, record:

- **Incorrect line or condition:**
- **Why it looked reasonable:**
- **Counterexample that breaks it:**
- **Corrected rule:**
- **How to recognize this mistake next time:**

## Pattern-level reminder

Store existence, frequency, index, or complement information about values already seen.
