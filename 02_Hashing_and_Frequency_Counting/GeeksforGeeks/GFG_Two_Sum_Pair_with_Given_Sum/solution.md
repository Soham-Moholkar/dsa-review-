# Two Sum - Pair with Given Sum — explained solution
...
- **Better:** Sort and use two pointers. Time O(n log n); space O(log n).
- **Optimal:** Hash-set complement lookup. Time O(n) average; space O(n).
...
```bash
python3 scripts/test_solutions.py --problem GFG_Two_Sum_Pair_with_Given_Sum --sanitize
```
