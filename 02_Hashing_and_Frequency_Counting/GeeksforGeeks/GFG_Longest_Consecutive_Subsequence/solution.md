# Longest Consecutive Subsequence — explained solution
...
- **Brute Force:** Search for every next value. Time O(n^3); space O(1).
- **Better:** Sort and scan. Time O(n log n); space O(log n).
- **Optimal:** Hash set and sequence starts. Time O(n) average; space O(n).
...
```bash
python3 scripts/test_solutions.py --problem GFG_Longest_Consecutive_Subsequence --sanitize
```
