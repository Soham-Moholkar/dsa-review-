...
This particular file uses the "Search for every next value" approach.
...
----------------------------
Approach name: Search for every next value
...
8. `int currentValue = arr[i];`
   Creates `currentValue` and initializes it from `arr[i]`. This gives the algorithm its starting state.
9. `int length = 1;`
   Creates `length` and initializes it from `1`. This gives the algorithm its starting state.
10. `while(currentValue != INT_MAX) {`
   Repeats the following block while `currentValue != INT_MAX` is true.
11. `bool found = false;`
   Creates `found` and initializes it from `false`. This gives the algorithm its starting state.
12. `for(int j = 0; j < arr.size(); j++) {`
   Starts a loop: first `int j = 0`; keep repeating while `j < arr.size()` is true; after each iteration perform `j++`.
13. `if(arr[j] == currentValue + 1) {`
   Runs the next block only when `arr[j] == currentValue + 1` is true.
14. `found = true;`
...
16. `if(!found) {`
   Runs the next block only when `!found` is true.
17. `break;`
   Stops the nearest loop immediately because no more iterations are needed.
18. `currentValue++;`
...
19. `length++;`
   Moves the relevant counter or pointer by one position.
20. `answer = max(answer, length);`
   Updates `answer` to `max(answer, length)` for the next step of the algorithm.
21. `return answer;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Search for every next value" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Search for every next value" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
