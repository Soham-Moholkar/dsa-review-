...
This particular file uses the "Use a separate unique vector" approach.
...
----------------------------
Approach name: Use a separate unique vector
...
6. `vector<int> uniqueValues;`
   Declares `uniqueValues` so it can store state used by the algorithm.
...
8. `if(i == 0 || arr[i] != arr[i - 1]) {`
   Runs the next block only when `i == 0 || arr[i] != arr[i - 1]` is true.
9. `uniqueValues.push_back(arr[i]);`
   Appends the computed value to the end of the result/container.
10. `for(int i = 0; i < uniqueValues.size(); i++) {`
   Starts a loop: first `int i = 0`; keep repeating while `i < uniqueValues.size()` is true; after each iteration perform `i++`.
11. `arr[i] = uniqueValues[i];`
   Updates `arr[i]` to `uniqueValues[i]` for the next step of the algorithm.
12. `return uniqueValues.size();`
   Ends the function and sends `uniqueValues.size()` back to the caller.
