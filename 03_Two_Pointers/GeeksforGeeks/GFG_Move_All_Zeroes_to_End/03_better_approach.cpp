...
This particular file uses the "Overwrite then fill" approach.
...
----------------------------
Approach name: Overwrite then fill
...
9. `arr[j] = arr[i];`
   Updates `arr[j]` to `arr[i]` for the next step of the algorithm.
10. `j++;`
   Moves the relevant counter or pointer by one position.
11. `while(j < arr.size()) {`
   Repeats the following block while `j < arr.size()` is true.
12. `arr[j] = 0;`
   Updates `arr[j]` to `0` for the next step of the algorithm.
13. `j++;`
   Moves the relevant counter or pointer by one position.
