...
This particular file uses the "Use a new vector" approach.
...
- `pushZerosToEnd` is the function name the online judge calls.
...
----------------------------
Approach name: Use a new vector
...
5. `void pushZerosToEnd(vector<int>& arr) {`
...
9. `answer.push_back(arr[i]);`
   Appends the computed value to the end of the result/container.
10. `while(answer.size() < arr.size()) {`
   Repeats the following block while `answer.size() < arr.size()` is true.
11. `answer.push_back(0);`
   Appends the computed value to the end of the result/container.
12. `arr = answer;`
   Updates `arr` to `answer` for the next step of the algorithm.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Use a new vector" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Use a new vector" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
