...
This particular file uses the "Check every pair" approach.
...
----------------------------
Approach name: Check every pair
...
7. `for(int i = 0; i < height.size(); i++) {`
   Starts a loop: first `int i = 0`; keep repeating while `i < height.size()` is true; after each iteration perform `i++`.
8. `for(int j = i + 1; j < height.size(); j++) {`
   Starts a loop: first `int j = i + 1`; keep repeating while `j < height.size()` is true; after each iteration perform `j++`.
9. `int area = min(height[i], height[j]) * (j - i);`
   Creates `area` and initializes it from `min(height[i], height[j]) * (j - i)`. This gives the algorithm its starting state.
10. `answer = max(answer, area);`
