...
This particular file uses the "Place values in an extra vector" approach.
...
----------------------------
Approach name: Place values in an extra vector
...
12. `int newIndex = (i + k) % n;`
   Creates `newIndex` and initializes it from `(i + k) % n`. This gives the algorithm its starting state.
13. `answer[newIndex] = nums[i];`
   Updates `answer[newIndex]` to `nums[i]` for the next step of the algorithm.
14. `nums = answer;`
   Updates `nums` to `answer` for the next step of the algorithm.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Place values in an extra vector" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Place values in an extra vector" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
