...
This particular file uses the "Rotate one step k times" approach.
...
----------------------------
Approach name: Rotate one step k times
...
10. `for(int step = 0; step < k; step++) {`
   Starts a loop: first `int step = 0`; keep repeating while `step < k` is true; after each iteration perform `step++`.
11. `int lastValue = nums[n - 1];`
   Creates `lastValue` and initializes it from `nums[n - 1]`. This gives the algorithm its starting state.
12. `for(int i = n - 1; i > 0; i--) {`
   Starts a loop: first `int i = n - 1`; keep repeating while `i > 0` is true; after each iteration perform `i--`.
13. `nums[i] = nums[i - 1];`
   Updates `nums[i]` to `nums[i - 1]` for the next step of the algorithm.
14. `nums[0] = lastValue;`
   Updates `nums[0]` to `lastValue` for the next step of the algorithm.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Rotate one step k times" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Rotate one step k times" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
