...
This particular file uses the "Overwrite non-zero values then fill zeroes" approach.
...
- `moveZeroes` is the function name the online judge calls.
...
----------------------------
Approach name: Overwrite non-zero values then fill zeroes
...
9. `nums[j] = nums[i];`
   Updates `nums[j]` to `nums[i]` for the next step of the algorithm.
10. `j++;`
   Moves the relevant counter or pointer by one position.
11. `while(j < nums.size()) {`
   Repeats the following block while `j < nums.size()` is true.
12. `nums[j] = 0;`
   Updates `nums[j]` to `0` for the next step of the algorithm.
13. `j++;`
   Moves the relevant counter or pointer by one position.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Overwrite non-zero values then fill zeroes" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Overwrite non-zero values then fill zeroes" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
