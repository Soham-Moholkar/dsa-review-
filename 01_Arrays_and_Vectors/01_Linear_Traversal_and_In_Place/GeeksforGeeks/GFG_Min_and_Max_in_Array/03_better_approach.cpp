...
This particular file uses the "Two separate scans" approach.
...
----------------------------
Approach name: Two separate scans
...
6. `int minimumValue = arr[0];`
   Creates `minimumValue` and initializes it from `arr[0]`. This gives the algorithm its starting state.
7. `int maximumValue = arr[0];`
   Creates `maximumValue` and initializes it from `arr[0]`. This gives the algorithm its starting state.
...
9. `if(arr[i] < minimumValue) {`
   Runs the next block only when `arr[i] < minimumValue` is true.
10. `minimumValue = arr[i];`
   Updates `minimumValue` to `arr[i]` for the next step of the algorithm.
11. `for(int i = 1; i < arr.size(); i++) {`
   Starts a loop: first `int i = 1`; keep repeating while `i < arr.size()` is true; after each iteration perform `i++`.
12. `if(arr[i] > maximumValue) {`
   Runs the next block only when `arr[i] > maximumValue` is true.
13. `maximumValue = arr[i];`
   Updates `maximumValue` to `arr[i]` for the next step of the algorithm.
14. `return {minimumValue, maximumValue};`
   Ends the function and sends `{minimumValue, maximumValue}` back to the caller.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Two separate scans" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Two separate scans" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
