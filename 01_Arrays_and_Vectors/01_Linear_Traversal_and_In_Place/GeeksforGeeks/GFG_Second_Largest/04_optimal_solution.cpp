...
This particular file uses the "One scan with two states" approach.
...
----------------------------
Approach name: One scan with two states
...
7. `long long secondLargest = LLONG_MIN;`
...
10. `secondLargest = largestValue;`
   Updates `secondLargest` to `largestValue` for the next step of the algorithm.
11. `largestValue = arr[i];`
   Updates `largestValue` to `arr[i]` for the next step of the algorithm.
12. `else if(arr[i] > secondLargest && arr[i] != largestValue) {`
   Defines the judge-facing function and lists the inputs it receives.
13. `secondLargest = arr[i];`
   Updates `secondLargest` to `arr[i]` for the next step of the algorithm.
14. `if(secondLargest == LLONG_MIN) {`
