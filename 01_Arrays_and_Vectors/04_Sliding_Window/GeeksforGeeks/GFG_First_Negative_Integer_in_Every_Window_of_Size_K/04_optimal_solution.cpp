...
This particular file uses the "Deque of negative indexes" approach.
...
----------------------------
Approach name: Deque of negative indexes
...
10. `negativeIndexes.push_back(i);`
   Appends the computed value to the end of the result/container.
11. `if(i >= K - 1) {`
   Runs the next block only when `i >= K - 1` is true.
12. `int windowStart = i - K + 1;`
   Creates `windowStart` and initializes it from `i - K + 1`. This gives the algorithm its starting state.
13. `while(!negativeIndexes.empty() && negativeIndexes.front() < windowStart) {`
   Repeats the following block while `!negativeIndexes.empty() && negativeIndexes.front() < windowStart` is true.
14. `negativeIndexes.pop_front();`
   Removes the oldest element from the front of the deque after it becomes irrelevant.
15. `if(negativeIndexes.empty()) {`
   Runs the next block only when `negativeIndexes.empty()` is true.
16. `answer.push_back(0);`
   Appends the computed value to the end of the result/container.
17. `else {`
   Handles the remaining case after the preceding condition(s) were false.
18. `answer.push_back(A[negativeIndexes.front()]);`
   Appends the computed value to the end of the result/container.
19. `return answer;`
