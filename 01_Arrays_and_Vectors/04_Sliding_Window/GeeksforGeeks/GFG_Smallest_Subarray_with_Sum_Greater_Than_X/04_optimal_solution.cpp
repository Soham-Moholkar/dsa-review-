...
This particular file uses the "Variable sliding window" approach.
...
----------------------------
Approach name: Variable sliding window
...
9. `for(int right = 0; right < arr.size(); right++) {`
   Starts a loop: first `int right = 0`; keep repeating while `right < arr.size()` is true; after each iteration perform `right++`.
10. `sum = sum + arr[right];`
   Updates `sum` to `sum + arr[right]` for the next step of the algorithm.
11. `while(sum > x) {`
   Repeats the following block while `sum > x` is true.
12. `answer = min(answer, right - left + 1);`
   Updates `answer` to `min(answer, right - left + 1)` for the next step of the algorithm.
13. `sum = sum - arr[left];`
   Updates `sum` to `sum - arr[left]` for the next step of the algorithm.
14. `left++;`
   Moves the relevant counter or pointer by one position.
15. `if(answer == INT_MAX) {`
   Runs the next block only when `answer == INT_MAX` is true.
16. `return 0;`
