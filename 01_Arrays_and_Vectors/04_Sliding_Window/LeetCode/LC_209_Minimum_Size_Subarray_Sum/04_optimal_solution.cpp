...
9. `for(int right = 0; right < nums.size(); right++) {`
   Starts a loop: first `int right = 0`; keep repeating while `right < nums.size()` is true; after each iteration perform `right++`.
10. `sum = sum + nums[right];`
   Updates `sum` to `sum + nums[right]` for the next step of the algorithm.
11. `while(sum >= target) {`
   Repeats the following block while `sum >= target` is true.
...
13. `sum = sum - nums[left];`
   Updates `sum` to `sum - nums[left]` for the next step of the algorithm.
14. `left++;`
   Moves the relevant counter or pointer by one position.
15. `return answer == INT_MAX ? 0 : answer;`
