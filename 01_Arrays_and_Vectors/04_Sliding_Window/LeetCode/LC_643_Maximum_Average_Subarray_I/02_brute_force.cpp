...
This particular file uses the "Recalculate every sum" approach.
...
----------------------------
Approach name: Recalculate every sum
...
6. `double answer = -DBL_MAX;`
   Starts a do-while loop, so its body runs at least once.
7. `for(int start = 0; start + k <= nums.size(); start++) {`
   Starts a loop: first `int start = 0`; keep repeating while `start + k <= nums.size()` is true; after each iteration perform `start++`.
8. `long long sum = 0;`
...
11. `answer = max(answer, (double)sum / k);`
   Updates `answer` to `max(answer, (double)sum / k)` for the next step of the algorithm.
12. `return answer;`
