...
This particular file uses the "Find pivot, then ordinary binary search" approach.
...
----------------------------
Approach name: Find pivot, then ordinary binary search
...
8. `int right = n - 1;`
   Creates `right` and initializes it from `n - 1`. This gives the algorithm its starting state.
9. `while(left < right) {`
...
11. `if(arr[mid] > arr[right]) {`
   Runs the next block only when `arr[mid] > arr[right]` is true.
12. `left = mid + 1;`
...
14. `right = mid;`
   Updates `right` to `mid` for the next step of the algorithm.
15. `int pivot = left;`
   Creates `pivot` and initializes it from `left`. This gives the algorithm its starting state.
16. `if(key >= arr[pivot] && key <= arr[n - 1]) {`
   Runs the next block only when `key >= arr[pivot] && key <= arr[n - 1]` is true.
17. `left = pivot;`
   Updates `left` to `pivot` for the next step of the algorithm.
18. `right = n - 1;`
   Updates `right` to `n - 1` for the next step of the algorithm.
19. `else {`
   Handles the remaining case after the preceding condition(s) were false.
20. `left = 0;`
   Updates `left` to `0` for the next step of the algorithm.
21. `right = pivot - 1;`
   Updates `right` to `pivot - 1` for the next step of the algorithm.
22. `while(left <= right) {`
   Repeats the following block while `left <= right` is true.
23. `int mid = left + (right - left) / 2;`
   Creates `mid` and initializes it from `left + (right - left) / 2`. This gives the algorithm its starting state.
24. `if(arr[mid] == key) return mid;`
   Runs the next block only when `arr[mid] == key` is true. The one-line action is `return mid;`.
25. `if(arr[mid] < key) left = mid + 1;`
   Runs the next block only when `arr[mid] < key` is true. The one-line action is `left = mid + 1;`.
26. `else right = mid - 1;`
   Handles the remaining case after the preceding condition(s) were false.
27. `return -1;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Find pivot, then ordinary binary search" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Find pivot, then ordinary binary search" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
