...
- **Better:** Find pivot, then ordinary binary search. Time O(log n); space O(1).
- **Optimal:** Modified binary search. Time O(log n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem GFG_Search_in_Rotated_Sorted_Array --sanitize
```
