# First and Last Occurrences — explained solution
...
- **Better:** lower_bound and upper_bound. Time O(log n); space O(1).
- **Optimal:** Two boundary binary searches. Time O(log n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem GFG_First_and_Last_Occurrences --sanitize
```
