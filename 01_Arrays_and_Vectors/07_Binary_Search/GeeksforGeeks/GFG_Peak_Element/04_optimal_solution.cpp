...
This particular file uses the "Binary search on slope" approach.
...
----------------------------
Approach name: Binary search on slope
...
10. `if(arr[mid] < arr[mid + 1]) {`
   Runs the next block only when `arr[mid] < arr[mid + 1]` is true.
11. `left = mid + 1;`
