...
This particular file uses the "Check every index and both neighbours" approach.
...
----------------------------
Approach name: Check every index and both neighbours
...
8. `bool leftOkay = i == 0 || arr[i] >= arr[i - 1];`
   Creates `leftOkay` and initializes it from `i == 0 || arr[i] >= arr[i - 1]`. This gives the algorithm its starting state.
9. `bool rightOkay = i == n - 1 || arr[i] >= arr[i + 1];`
   Creates `rightOkay` and initializes it from `i == n - 1 || arr[i] >= arr[i + 1]`. This gives the algorithm its starting state.
10. `if(leftOkay && rightOkay) {`
   Runs the next block only when `leftOkay && rightOkay` is true.
11. `return i;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Check every index and both neighbours" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Check every index and both neighbours" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
