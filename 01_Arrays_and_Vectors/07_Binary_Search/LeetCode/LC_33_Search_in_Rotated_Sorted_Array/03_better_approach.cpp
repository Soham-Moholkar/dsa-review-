...
This particular file uses the "Pivot plus binary search" approach.
...
----------------------------
Approach name: Pivot plus binary search
...
11. `if(nums[mid] > nums[right]) left = mid + 1;`
   Runs the next block only when `nums[mid] > nums[right]` is true. The one-line action is `left = mid + 1;`.
12. `else right = mid;`
...
14. `if(target >= nums[pivot] && target <= nums[n - 1]) {`
   Runs the next block only when `target >= nums[pivot] && target <= nums[n - 1]` is true.
15. `left = pivot;`
...
22. `if(nums[mid] == target) return mid;`
   Runs the next block only when `nums[mid] == target` is true. The one-line action is `return mid;`.
23. `if(nums[mid] < target) left = mid + 1;`
   Runs the next block only when `nums[mid] < target` is true. The one-line action is `left = mid + 1;`.
24. `else right = mid - 1;`
   Handles the remaining case after the preceding condition(s) were false.
25. `return -1;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Pivot plus binary search" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Pivot plus binary search" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
