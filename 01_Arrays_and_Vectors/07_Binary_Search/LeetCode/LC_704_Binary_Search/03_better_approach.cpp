...
This particular file uses the "lower_bound" approach.
...
6. `auto position = lower_bound(nums.begin(), nums.end(), target);`
   Creates `position` and initializes it from `lower_bound(nums.begin(), nums.end(), target)`. This gives the algorithm its starting state.
7. `if(position != nums.end() && *position == target) {`
   Runs the next block only when `position != nums.end() && *position == target` is true.
8. `return position - nums.begin();`
   Ends the function and sends `position - nums.begin()` back to the caller.
9. `return -1;`
