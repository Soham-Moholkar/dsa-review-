...
This particular file uses the "Neighbour checks" approach.
...
----------------------------
Approach name: Neighbour checks
...
7. `bool leftOkay = i == 0 || nums[i] > nums[i - 1];`
   Creates `leftOkay` and initializes it from `i == 0 || nums[i] > nums[i - 1]`. This gives the algorithm its starting state.
8. `bool rightOkay = i == nums.size() - 1 || nums[i] > nums[i + 1];`
   Creates `rightOkay` and initializes it from `i == nums.size() - 1 || nums[i] > nums[i + 1]`. This gives the algorithm its starting state.
9. `if(leftOkay && rightOkay) return i;`
   Runs the next block only when `leftOkay && rightOkay` is true. The one-line action is `return i;`.
10. `return -1;`
