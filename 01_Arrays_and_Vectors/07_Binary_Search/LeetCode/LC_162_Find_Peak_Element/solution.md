# Find Peak Element — explained solution
...
- **Brute Force:** Neighbour checks. Time O(n); space O(1).
- **Better:** First falling edge. Time O(n); space O(1).
- **Optimal:** Binary search on adjacent slope. Time O(log n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem LC_162_Find_Peak_Element --sanitize
```
