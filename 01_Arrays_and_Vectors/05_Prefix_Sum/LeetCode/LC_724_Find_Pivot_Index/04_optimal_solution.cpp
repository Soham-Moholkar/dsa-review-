...
This particular file uses the "Total sum and left sum" approach.
...
----------------------------
Approach name: Total sum and left sum
...
14. `leftSum = leftSum + nums[i];`
   Updates `leftSum` to `leftSum + nums[i]` for the next step of the algorithm.
15. `return -1;`
