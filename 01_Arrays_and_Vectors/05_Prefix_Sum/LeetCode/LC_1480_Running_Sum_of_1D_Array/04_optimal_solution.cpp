...
This particular file uses the "Modify the input in place" approach.
...
----------------------------
Approach name: Modify the input in place
...
7. `nums[i] = nums[i] + nums[i - 1];`
   Updates `nums[i]` to `nums[i] + nums[i - 1]` for the next step of the algorithm.
8. `return nums;`
   Ends the function and sends `nums` back to the caller.
