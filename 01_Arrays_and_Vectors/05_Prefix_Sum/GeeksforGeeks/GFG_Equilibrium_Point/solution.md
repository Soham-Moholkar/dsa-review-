# Equilibrium Point — explained solution
...
- **Better:** Prefix and suffix arrays. Time O(n); space O(n).
- **Optimal:** Total sum and running left sum. Time O(n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem GFG_Equilibrium_Point --sanitize
```
