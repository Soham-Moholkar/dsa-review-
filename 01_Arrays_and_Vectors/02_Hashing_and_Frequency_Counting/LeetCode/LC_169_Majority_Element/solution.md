...
- **Brute Force:** Count each value. Time O(n^2); space O(1).
- **Better:** Frequency hashmap. Time O(n) average; space O(n).
- **Optimal:** Boyer-Moore voting. Time O(n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem LC_169_Majority_Element --sanitize
```
