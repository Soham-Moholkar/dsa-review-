...
This particular file uses the "Boyer-Moore voting" approach.
...
----------------------------
Approach name: Boyer-Moore voting
...
6. `int candidate = nums[0];`
   Creates `candidate` and initializes it from `nums[0]`. This gives the algorithm its starting state.
7. `int count = 0;`
...
10. `candidate = nums[i];`
   Updates `candidate` to `nums[i]` for the next step of the algorithm.
11. `if(nums[i] == candidate) {`
   Runs the next block only when `nums[i] == candidate` is true.
12. `count++;`
