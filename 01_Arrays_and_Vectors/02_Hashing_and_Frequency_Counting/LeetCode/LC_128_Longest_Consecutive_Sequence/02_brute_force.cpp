...
This particular file uses the "Repeated linear existence search" approach.
...
----------------------------
Approach name: Repeated linear existence search
...
8. `int currentValue = nums[i];`
   Creates `currentValue` and initializes it from `nums[i]`. This gives the algorithm its starting state.
9. `int length = 1;`
   Creates `length` and initializes it from `1`. This gives the algorithm its starting state.
10. `while(currentValue != INT_MAX && find(nums.begin(), nums.end(), currentValue + 1) != nums.end()) {`
   Repeats the following block while `currentValue != INT_MAX && find(nums.begin(), nums.end(), currentValue + 1) != nums.end()` is true.
11. `currentValue++;`
   Moves the relevant counter or pointer by one position.
12. `length++;`
   Moves the relevant counter or pointer by one position.
13. `answer = max(answer, length);`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Repeated linear existence search" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Repeated linear existence search" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
