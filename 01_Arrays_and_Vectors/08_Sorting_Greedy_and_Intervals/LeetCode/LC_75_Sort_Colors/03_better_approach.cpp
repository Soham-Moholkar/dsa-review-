...
This particular file uses the "Counting" approach.
...
6. `int count[3] = {0,0,0};`
   Creates `the variable` and initializes it from `{0,0,0}`. This gives the algorithm its starting state.
7. `for(int i = 0; i < nums.size(); i++) count[nums[i]]++;`
   Starts a loop: first `int i = 0`; keep repeating while `i < nums.size()` is true; after each iteration perform `i++`. Its one-line body is `count[nums[i]]++;`.
8. `int index = 0;`
   Creates `index` and initializes it from `0`. This gives the algorithm its starting state.
9. `for(int value = 0; value <= 2; value++) {`
   Starts a loop: first `int value = 0`; keep repeating while `value <= 2` is true; after each iteration perform `value++`.
10. `while(count[value] > 0) {`
   Repeats the following block while `count[value] > 0` is true.
11. `nums[index++] = value;`
   Updates `nums[index++]` to `value` for the next step of the algorithm.
12. `count[value]--;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Counting" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Counting" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
