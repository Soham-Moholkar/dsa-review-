...
This particular file uses the "Append, sort, merge" approach.
...
----------------------------
Approach name: Append, sort, merge
...
6. `intervals.push_back(newInterval);`
...
9. `for(int i = 0; i < intervals.size(); i++) {`
   Starts a loop: first `int i = 0`; keep repeating while `i < intervals.size()` is true; after each iteration perform `i++`.
10. `if(answer.empty() || intervals[i][0] > answer.back()[1]) answer.push_back(intervals[i]);`
   Runs the next block only when `answer.empty() || intervals[i][0] > answer.back()[1]` is true. The one-line action is `answer.push_back(intervals[i]);`.
11. `else answer.back()[1] = max(answer.back()[1], intervals[i][1]);`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Append, sort, merge" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Append, sort, merge" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
