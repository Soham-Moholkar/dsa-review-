# Insert Interval — explained solution
...
- **Brute Force:** Append, sort, merge. Time O(n log n); space O(n).
- **Better:** Three-phase scan. Time O(n); space O(n).
- **Optimal:** Linear insertion and merge. Time O(n); space O(n) answer.
...
```bash
python3 scripts/test_solutions.py --problem LC_57_Insert_Interval --sanitize
```
