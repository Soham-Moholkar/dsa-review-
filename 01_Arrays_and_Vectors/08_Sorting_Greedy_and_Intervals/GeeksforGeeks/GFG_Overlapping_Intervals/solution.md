# Overlapping Intervals — explained solution
...
- **Brute Force:** Repeatedly merge any overlapping pair. Time O(n^3); space O(n).
- **Better:** Sort and merge into answer. Time O(n log n); space O(n).
- **Optimal:** Sorted one-pass interval merge. Time O(n log n); space O(n) answer.
...
```bash
python3 scripts/test_solutions.py --problem GFG_Overlapping_Intervals --sanitize
```
