# Maximum Subarray — explained solution
...
- **Brute Force:** Triple-loop enumeration. Time O(n^3); space O(1).
- **Better:** Running sum per start. Time O(n^2); space O(1).
- **Optimal:** Kadane algorithm. Time O(n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem LC_53_Maximum_Subarray --sanitize
```
