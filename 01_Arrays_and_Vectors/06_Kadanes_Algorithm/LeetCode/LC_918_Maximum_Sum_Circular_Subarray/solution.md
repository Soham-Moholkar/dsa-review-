# Maximum Sum Circular Subarray — explained solution
...
- **Better:** Duplicate and enumerate bounded ranges. Time O(n^2); space O(n).
- **Optimal:** Maximum and minimum Kadane. Time O(n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem LC_918_Maximum_Sum_Circular_Subarray --sanitize
```
