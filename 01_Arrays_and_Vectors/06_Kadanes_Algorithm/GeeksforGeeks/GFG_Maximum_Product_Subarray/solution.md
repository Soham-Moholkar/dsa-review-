...
- **Brute Force:** Enumerate all products. Time O(n^2); space O(1).
- **Better:** Prefix and suffix product scans. Time O(n); space O(1).
- **Optimal:** Track maximum and minimum ending products. Time O(n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem GFG_Maximum_Product_Subarray --sanitize
```
