...
This particular file uses the "Four-boundary spiral" approach.
...
----------------------------
Approach name: Four-boundary spiral
...
8. `int bottom = matrix.size() - 1;`
   Creates `bottom` and initializes it from `matrix.size() - 1`. This gives the algorithm its starting state.
9. `int left = 0;`
   Creates `left` and initializes it from `0`. This gives the algorithm its starting state.
10. `int right = matrix[0].size() - 1;`
   Creates `right` and initializes it from `matrix[0].size() - 1`. This gives the algorithm its starting state.
11. `while(top <= bottom && left <= right) {`
   Repeats the following block while `top <= bottom && left <= right` is true.
12. `for(int column = left; column <= right; column++) answer.push_back(matrix[top][column]);`
   Starts a loop: first `int column = left`; keep repeating while `column <= right` is true; after each iteration perform `column++`. Its one-line body is `answer.push_back(matrix[top][column]);`.
13. `top++;`
   Moves the relevant counter or pointer by one position.
14. `for(int row = top; row <= bottom; row++) answer.push_back(matrix[row][right]);`
   Starts a loop: first `int row = top`; keep repeating while `row <= bottom` is true; after each iteration perform `row++`. Its one-line body is `answer.push_back(matrix[row][right]);`.
15. `right--;`
   Moves the relevant counter or pointer by one position.
16. `if(top <= bottom) {`
   Runs the next block only when `top <= bottom` is true.
17. `for(int column = right; column >= left; column--) answer.push_back(matrix[bottom][column]);`
   Starts a loop: first `int column = right`; keep repeating while `column >= left` is true; after each iteration perform `column--`. Its one-line body is `answer.push_back(matrix[bottom][column]);`.
18. `bottom--;`
   Moves the relevant counter or pointer by one position.
19. `if(left <= right) {`
   Runs the next block only when `left <= right` is true.
20. `for(int row = bottom; row >= top; row--) answer.push_back(matrix[row][left]);`
   Starts a loop: first `int row = bottom`; keep repeating while `row >= top` is true; after each iteration perform `row--`. Its one-line body is `answer.push_back(matrix[row][left]);`.
21. `left++;`
   Moves the relevant counter or pointer by one position.
22. `return answer;`
