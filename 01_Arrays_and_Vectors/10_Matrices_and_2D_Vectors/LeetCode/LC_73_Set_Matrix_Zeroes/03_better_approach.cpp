...
8. `vector<bool> zeroRow(rows, false);`
   Declares `zeroRow` so it can store state used by the algorithm.
9. `vector<bool> zeroColumn(columns, false);`
   Declares `zeroColumn` so it can store state used by the algorithm.
...
12. `if(matrix[row][column] == 0) {`
   Runs the next block only when `matrix[row][column] == 0` is true.
13. `zeroRow[row] = true;`
   Updates `zeroRow[row]` to `true` for the next step of the algorithm.
14. `zeroColumn[column] = true;`
   Updates `zeroColumn[column]` to `true` for the next step of the algorithm.
15. `for(int row = 0; row < rows; row++) {`
   Starts a loop: first `int row = 0`; keep repeating while `row < rows` is true; after each iteration perform `row++`.
16. `for(int column = 0; column < columns; column++) {`
   Starts a loop: first `int column = 0`; keep repeating while `column < columns` is true; after each iteration perform `column++`.
17. `if(zeroRow[row] || zeroColumn[column]) matrix[row][column] = 0;`
   Runs the next block only when `zeroRow[row] || zeroColumn[column]` is true. The one-line action is `matrix[row][column] = 0;`.
