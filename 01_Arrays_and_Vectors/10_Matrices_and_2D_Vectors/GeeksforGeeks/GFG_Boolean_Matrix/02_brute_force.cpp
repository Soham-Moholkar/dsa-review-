...
This particular file uses the "Use an unchanged copy for every marker" approach.
...
----------------------------
Approach name: Use an unchanged copy for every marker
...
11. `if(original[row][column] == 1) {`
   Runs the next block only when `original[row][column] == 1` is true.
12. `for(int c = 0; c < columns; c++) matrix[row][c] = 1;`
   Starts a loop: first `int c = 0`; keep repeating while `c < columns` is true; after each iteration perform `c++`. Its one-line body is `matrix[row][c] = 1;`.
13. `for(int r = 0; r < rows; r++) matrix[r][column] = 1;`
   Starts a loop: first `int r = 0`; keep repeating while `r < rows` is true; after each iteration perform `r++`. Its one-line body is `matrix[r][column] = 1;`.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Use an unchanged copy for every marker" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Use an unchanged copy for every marker" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
