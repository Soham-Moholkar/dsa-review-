...
This particular file uses the "Check every cell" approach.
...
----------------------------
Approach name: Check every cell
...
5. `bool searchMatrix(vector<vector<int>>& mat, int x) {`
   Defines the judge-facing function and lists the inputs it receives.
6. `for(int row = 0; row < mat.size(); row++) {`
   Starts a loop: first `int row = 0`; keep repeating while `row < mat.size()` is true; after each iteration perform `row++`.
7. `for(int column = 0; column < mat[0].size(); column++) {`
   Starts a loop: first `int column = 0`; keep repeating while `column < mat[0].size()` is true; after each iteration perform `column++`.
8. `if(mat[row][column] == x) return true;`
   Runs the next block only when `mat[row][column] == x` is true. The one-line action is `return true;`.
9. `return false;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Check every cell" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Check every cell" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
