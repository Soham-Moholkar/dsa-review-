...
This particular file uses the "Binary search every row" approach.
...
----------------------------
Approach name: Binary search every row
...
7. `if(binary_search(mat[row].begin(), mat[row].end(), x)) return true;`
   Runs the next block only when `binary_search(mat[row].begin(), mat[row].end(), x)` is true. The one-line action is `return true;`.
8. `return false;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Binary search every row" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Binary search every row" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
