...
This particular file uses the "Transpose then reverse columns" approach.
...
- `rotateby90` is the function name the online judge calls.
...
----------------------------
Approach name: Transpose then reverse columns
...
5. `void rotateby90(vector<vector<int>>& mat) {`
...
9. `swap(mat[row][column], mat[column][row]);`
   Exchanges the two selected values without needing a manual temporary variable.
10. `for(int column = 0; column < n; column++) {`
   Starts a loop: first `int column = 0`; keep repeating while `column < n` is true; after each iteration perform `column++`.
11. `int top = 0;`
   Creates `top` and initializes it from `0`. This gives the algorithm its starting state.
12. `int bottom = n - 1;`
   Creates `bottom` and initializes it from `n - 1`. This gives the algorithm its starting state.
13. `while(top < bottom) {`
   Repeats the following block while `top < bottom` is true.
14. `swap(mat[top][column], mat[bottom][column]);`
   Exchanges the two selected values without needing a manual temporary variable.
15. `top++;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Transpose then reverse columns" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Transpose then reverse columns" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
