# Spirally Traversing a Matrix — explained solution
...
- **Brute Force:** Simulation with visited cells and directions. Time O(rows × columns); space O(rows × columns).
- **Better:** Boundary traversal. Time O(rows × columns); space O(1) excluding answer.
- **Optimal:** Four shrinking boundaries. Time O(rows × columns); space O(1) excluding answer.
...
```bash
python3 scripts/test_solutions.py --problem GFG_Spirally_Traversing_a_Matrix --sanitize
```
