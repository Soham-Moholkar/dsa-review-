...
This particular file uses the "Fix two values and use a hash set" approach.
...
----------------------------
Approach name: Fix two values and use a hash set
...
10. `int required = -nums[i] - nums[j];`
   Creates `required` and initializes it from `-nums[i] - nums[j]`. This gives the algorithm its starting state.
11. `if(seen.find(required) != seen.end()) {`
   Runs the next block only when `seen.find(required) != seen.end()` is true.
12. `vector<int> triplet = {nums[i], nums[j], required};`
   Creates `triplet` and initializes it from `{nums[i], nums[j], required}`. This gives the algorithm its starting state.
13. `sort(triplet.begin(), triplet.end());`
   Sorts the selected range in ascending order, changing the container so ordered reasoning becomes possible.
14. `uniqueTriplets.insert(triplet);`
   Stores this value in the set/map so later iterations can find it.
15. `seen.insert(nums[j]);`
   Stores this value in the set/map so later iterations can find it.
16. `return vector<vector<int>>(uniqueTriplets.begin(), uniqueTriplets.end());`
...
- Extra-space complexity: O(n) excluding answers.
