...
This particular file uses the "Two pointers with equal-height skip" approach.
...
----------------------------
Approach name: Two pointers with equal-height skip
...
10. `int smallerHeight = min(height[left], height[right]);`
   Creates `smallerHeight` and initializes it from `min(height[left], height[right])`. This gives the algorithm its starting state.
11. `answer = max(answer, smallerHeight * (right - left));`
   Updates `answer` to `max(answer, smallerHeight * (right - left))` for the next step of the algorithm.
12. `while(left < right && height[left] <= smallerHeight) {`
   Repeats the following block while `left < right && height[left] <= smallerHeight` is true.
13. `left++;`
   Moves the relevant counter or pointer by one position.
14. `while(left < right && height[right] <= smallerHeight) {`
   Repeats the following block while `left < right && height[right] <= smallerHeight` is true.
15. `right--;`
   Moves the relevant counter or pointer by one position.
16. `return answer;`
