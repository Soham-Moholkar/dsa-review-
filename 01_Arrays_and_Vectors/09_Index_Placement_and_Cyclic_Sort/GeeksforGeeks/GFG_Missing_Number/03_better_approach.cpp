...
This particular file uses the "Sort and find first mismatch" approach.
...
----------------------------
Approach name: Sort and find first mismatch
...
8. `if(arr[i] != i + 1) return i + 1;`
   Runs the next block only when `arr[i] != i + 1` is true. The one-line action is `return i + 1;`.
9. `return arr.size() + 1;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Sort and find first mismatch" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Sort and find first mismatch" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
