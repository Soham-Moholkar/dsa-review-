...
This particular file uses the "In-place negative frequency counting" approach.
...
----------------------------
Approach name: In-place negative frequency counting
...
7. `for (int &value : arr) ++value;`
   Starts a range-based loop. `int &value : arr` means: take each element from the container in turn and run the block. Its one-line body is `++value;`.
8. `int i = 0;`
...
22. `for (int value = 0; value < n; ++value) {`
   Starts a loop: first `int value = 0`; keep repeating while `value < n` is true; after each iteration perform `++value`.
23. `if (arr[value] < -1) answer.push_back(value);`
   Runs the next block only when `arr[value] < -1` is true. The one-line action is `answer.push_back(value);`.
24. `if (answer.empty()) answer.push_back(-1);`
