...
9. `if(nums[i] >= 1 && nums[i] <= n && nums[i] != nums[nums[i] - 1]) {`
   Runs the next block only when `nums[i] >= 1 && nums[i] <= n && nums[i] != nums[nums[i] - 1]` is true.
...
14. `if(nums[i] != i + 1) return i + 1;`
   Runs the next block only when `nums[i] != i + 1` is true. The one-line action is `return i + 1;`.
15. `return n + 1;`
