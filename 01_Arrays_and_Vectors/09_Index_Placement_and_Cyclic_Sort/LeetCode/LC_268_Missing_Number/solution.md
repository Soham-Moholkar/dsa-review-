...
- **Better:** Sort and compare index to value. Time O(n log n); space O(log n).
- **Optimal:** XOR indexes and values. Time O(n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem LC_268_Missing_Number --sanitize
```
