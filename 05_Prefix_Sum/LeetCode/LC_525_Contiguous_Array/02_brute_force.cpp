...
This particular file uses the "Count zeroes and ones for every subarray" approach.
...
----------------------------
Approach name: Count zeroes and ones for every subarray
...
10. `for(int end = start; end < nums.size(); end++) {`
...
14. `ones++;`
   Moves the relevant counter or pointer by one position.
15. `if(zeroes == ones) {`
   Runs the next block only when `zeroes == ones` is true.
16. `answer = max(answer, end - start + 1);`
