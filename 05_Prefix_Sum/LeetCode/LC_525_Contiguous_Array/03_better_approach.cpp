...
This particular file uses the "Ordered map of transformed prefixes" approach.
...
- `findMaxLength` is the function name the online judge calls.
...
----------------------------
Approach name: Ordered map of transformed prefixes
...
11. `prefixSum = prefixSum + (nums[i] == 0 ? -1 : 1);`
   Performs this operation to maintain the state described in the algorithm walkthrough.
12. `if(firstIndex.find(prefixSum) != firstIndex.end()) {`
   Runs the next block only when `firstIndex.find(prefixSum) != firstIndex.end()` is true.
13. `answer = max(answer, i - firstIndex[prefixSum]);`
   Updates `answer` to `max(answer, i - firstIndex[prefixSum])` for the next step of the algorithm.
14. `else {`
   Handles the remaining case after the preceding condition(s) were false.
15. `firstIndex[prefixSum] = i;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Ordered map of transformed prefixes" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Ordered map of transformed prefixes" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
