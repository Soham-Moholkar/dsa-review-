...
This particular file uses the "Recalculate both sides" approach.
...
- `pivotIndex` is the function name the online judge calls.
...
----------------------------
Approach name: Recalculate both sides
...
8. `int rightSum = 0;`
   Creates `rightSum` and initializes it from `0`. This gives the algorithm its starting state.
9. `for(int j = 0; j < i; j++) {`
   Starts a loop: first `int j = 0`; keep repeating while `j < i` is true; after each iteration perform `j++`.
10. `leftSum = leftSum + nums[j];`
   Updates `leftSum` to `leftSum + nums[j]` for the next step of the algorithm.
...
12. `rightSum = rightSum + nums[j];`
   Updates `rightSum` to `rightSum + nums[j]` for the next step of the algorithm.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Recalculate both sides" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Recalculate both sides" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
