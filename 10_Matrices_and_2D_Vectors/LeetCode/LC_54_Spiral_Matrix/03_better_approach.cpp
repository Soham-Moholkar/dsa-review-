...
7. `int top = 0, bottom = matrix.size()-1, left = 0, right = matrix[0].size()-1;`
   Creates `top` and initializes it from `0, bottom = matrix.size()-1, left = 0, right = matrix[0].size()-1`. This gives the algorithm its starting state.
8. `while(top <= bottom && left <= right) {`
   Repeats the following block while `top <= bottom && left <= right` is true.
9. `for(int c = left; c <= right; c++) answer.push_back(matrix[top][c]);`
   Starts a loop: first `int c = left`; keep repeating while `c <= right` is true; after each iteration perform `c++`. Its one-line body is `answer.push_back(matrix[top][c]);`.
10. `top++;`
   Moves the relevant counter or pointer by one position.
11. `for(int r = top; r <= bottom; r++) answer.push_back(matrix[r][right]);`
   Starts a loop: first `int r = top`; keep repeating while `r <= bottom` is true; after each iteration perform `r++`. Its one-line body is `answer.push_back(matrix[r][right]);`.
12. `right--;`
   Moves the relevant counter or pointer by one position.
13. `if(top <= bottom) {`
   Runs the next block only when `top <= bottom` is true.
14. `for(int c = right; c >= left; c--) answer.push_back(matrix[bottom][c]);`
   Starts a loop: first `int c = right`; keep repeating while `c >= left` is true; after each iteration perform `c--`. Its one-line body is `answer.push_back(matrix[bottom][c]);`.
15. `bottom--;`
   Moves the relevant counter or pointer by one position.
16. `if(left <= right) {`
   Runs the next block only when `left <= right` is true.
17. `for(int r = bottom; r >= top; r--) answer.push_back(matrix[r][left]);`
   Starts a loop: first `int r = bottom`; keep repeating while `r >= top` is true; after each iteration perform `r--`. Its one-line body is `answer.push_back(matrix[r][left]);`.
18. `left++;`
   Moves the relevant counter or pointer by one position.
19. `return answer;`
