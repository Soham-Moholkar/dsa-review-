...
7. `int column = matrix[0].size() - 1;`
   Creates `column` and initializes it from `matrix[0].size() - 1`. This gives the algorithm its starting state.
8. `while(row < matrix.size() && column >= 0) {`
   Repeats the following block while `row < matrix.size() && column >= 0` is true.
9. `if(matrix[row][column] == target) return true;`
   Runs the next block only when `matrix[row][column] == target` is true. The one-line action is `return true;`.
10. `else if(matrix[row][column] > target) column--;`
   If earlier branches failed, runs this block when `matrix[row][column] > target` is true. The one-line action is `column--;`.
11. `else row++;`
