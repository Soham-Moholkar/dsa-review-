# Search a 2D Matrix II — explained solution
...
- **Brute Force:** Scan every cell. Time O(r × c); space O(1).
- **Better:** Binary search each row. Time O(r log c); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem LC_240_Search_a_2D_Matrix_II --sanitize
```
