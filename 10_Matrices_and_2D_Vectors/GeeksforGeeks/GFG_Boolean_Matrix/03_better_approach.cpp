...
This particular file uses the "Row and column marker arrays" approach.
...
----------------------------
Approach name: Row and column marker arrays
...
8. `vector<bool> markRow(rows, false);`
   Declares `markRow` so it can store state used by the algorithm.
9. `vector<bool> markColumn(columns, false);`
   Declares `markColumn` so it can store state used by the algorithm.
...
12. `if(matrix[row][column] == 1) {`
   Runs the next block only when `matrix[row][column] == 1` is true.
13. `markRow[row] = true;`
   Updates `markRow[row]` to `true` for the next step of the algorithm.
14. `markColumn[column] = true;`
   Updates `markColumn[column]` to `true` for the next step of the algorithm.
15. `for(int row = 0; row < rows; row++) {`
   Starts a loop: first `int row = 0`; keep repeating while `row < rows` is true; after each iteration perform `row++`.
16. `for(int column = 0; column < columns; column++) {`
   Starts a loop: first `int column = 0`; keep repeating while `column < columns` is true; after each iteration perform `column++`.
17. `if(markRow[row] || markColumn[column]) matrix[row][column] = 1;`
   Runs the next block only when `markRow[row] || markColumn[column]` is true. The one-line action is `matrix[row][column] = 1;`.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Row and column marker arrays" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Row and column marker arrays" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
