...
This particular file uses the "Use first row and column as markers" approach.
...
- `booleanMatrix` is the function name the online judge calls.
...
----------------------------
Approach name: Use first row and column as markers
...
8. `bool firstRow = false;`
   Creates `firstRow` and initializes it from `false`. This gives the algorithm its starting state.
9. `bool firstColumn = false;`
   Creates `firstColumn` and initializes it from `false`. This gives the algorithm its starting state.
10. `for(int column = 0; column < columns; column++) if(matrix[0][column] == 1) firstRow = true;`
   Starts a loop: first `int column = 0`; keep repeating while `column < columns` is true; after each iteration perform `column++`. Its one-line body is `if(matrix[0][column] == 1) firstRow = true;`.
11. `for(int row = 0; row < rows; row++) if(matrix[row][0] == 1) firstColumn = true;`
   Starts a loop: first `int row = 0`; keep repeating while `row < rows` is true; after each iteration perform `row++`. Its one-line body is `if(matrix[row][0] == 1) firstColumn = true;`.
...
14. `if(matrix[row][column] == 1) {`
   Runs the next block only when `matrix[row][column] == 1` is true.
15. `matrix[row][0] = 1;`
   Updates `matrix[row][0]` to `1` for the next step of the algorithm.
16. `matrix[0][column] = 1;`
   Updates `matrix[0][column]` to `1` for the next step of the algorithm.
17. `for(int row = 1; row < rows; row++) {`
   Starts a loop: first `int row = 1`; keep repeating while `row < rows` is true; after each iteration perform `row++`.
18. `for(int column = 1; column < columns; column++) {`
   Starts a loop: first `int column = 1`; keep repeating while `column < columns` is true; after each iteration perform `column++`.
19. `if(matrix[row][0] == 1 || matrix[0][column] == 1) matrix[row][column] = 1;`
   Runs the next block only when `matrix[row][0] == 1 || matrix[0][column] == 1` is true. The one-line action is `matrix[row][column] = 1;`.
20. `if(firstRow) for(int column = 0; column < columns; column++) matrix[0][column] = 1;`
   Runs the next block only when `firstRow` is true. The one-line action is `for(int column = 0; column < columns; column++) matrix[0][column] = 1;`.
21. `if(firstColumn) for(int row = 0; row < rows; row++) matrix[row][0] = 1;`
   Runs the next block only when `firstColumn` is true. The one-line action is `for(int row = 0; row < rows; row++) matrix[row][0] = 1;`.
