...
This particular file uses the "Top-right staircase search" approach.
...
----------------------------
Approach name: Top-right staircase search
...
7. `int column = mat[0].size() - 1;`
   Creates `column` and initializes it from `mat[0].size() - 1`. This gives the algorithm its starting state.
8. `while(row < mat.size() && column >= 0) {`
   Repeats the following block while `row < mat.size() && column >= 0` is true.
9. `if(mat[row][column] == x) return true;`
   Runs the next block only when `mat[row][column] == x` is true. The one-line action is `return true;`.
10. `else if(mat[row][column] > x) column--;`
   If earlier branches failed, runs this block when `mat[row][column] > x` is true. The one-line action is `column--;`.
11. `else row++;`
