# Rotate by 90 Degree — explained solution
...
- **Better:** Transpose then reverse columns. Time O(n^2); space O(1).
- **Optimal:** In-place transpose and column reversal. Time O(n^2); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem GFG_Rotate_by_90_Degree --sanitize
```
