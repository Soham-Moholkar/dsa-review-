...
This particular file uses the "Use an extra matrix" approach.
...
----------------------------
Approach name: Use an extra matrix
...
7. `vector<vector<int>> answer(n, vector<int>(n));`
...
9. `for(int column = 0; column < n; column++) {`
   Starts a loop: first `int column = 0`; keep repeating while `column < n` is true; after each iteration perform `column++`.
10. `answer[n - column - 1][row] = mat[row][column];`
   Updates `answer[n - column - 1][row]` to `mat[row][column]` for the next step of the algorithm.
11. `mat = answer;`
   Updates `mat` to `answer` for the next step of the algorithm.
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Use an extra matrix" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Use an extra matrix" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
