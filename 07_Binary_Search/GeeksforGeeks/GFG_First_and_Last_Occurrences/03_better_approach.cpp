...
This particular file uses the "lower_bound and upper_bound" approach.
...
----------------------------
Approach name: lower_bound and upper_bound
...
6. `int first = lower_bound(arr.begin(), arr.end(), x) - arr.begin();`
   Creates `first` and initializes it from `lower_bound(arr.begin(), arr.end(), x) - arr.begin()`. This gives the algorithm its starting state.
7. `if(first == arr.size() || arr[first] != x) {`
   Runs the next block only when `first == arr.size() || arr[first] != x` is true.
8. `return {-1, -1};`
   Ends the function and sends `{-1, -1}` back to the caller.
9. `int last = upper_bound(arr.begin(), arr.end(), x) - arr.begin() - 1;`
   Creates `last` and initializes it from `upper_bound(arr.begin(), arr.end(), x) - arr.begin() - 1`. This gives the algorithm its starting state.
