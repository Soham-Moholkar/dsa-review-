...
- **Better:** Standard lower_bound lookup. Time O(log n); space O(1).
- **Optimal:** Manual iterative binary search. Time O(log n); space O(1).
...
```bash
python3 scripts/test_solutions.py --problem GFG_Binary_Search --sanitize
```
