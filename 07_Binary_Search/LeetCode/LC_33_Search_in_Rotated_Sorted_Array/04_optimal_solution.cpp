...
This particular file uses the "One modified binary search" approach.
...
----------------------------
Approach name: One modified binary search
...
11. `if(nums[left] <= nums[mid]) {`
   Runs the next block only when `nums[left] <= nums[mid]` is true.
12. `if(nums[left] <= target && target < nums[mid]) right = mid - 1;`
   Runs the next block only when `nums[left] <= target && target < nums[mid]` is true. The one-line action is `right = mid - 1;`.
13. `else left = mid + 1;`
   Handles the remaining case after the preceding condition(s) were false.
14. `else {`
   Handles the remaining case after the preceding condition(s) were false.
15. `if(nums[mid] < target && target <= nums[right]) left = mid + 1;`
   Runs the next block only when `nums[mid] < target && target <= nums[right]` is true. The one-line action is `left = mid + 1;`.
16. `else right = mid - 1;`
   Handles the remaining case after the preceding condition(s) were false.
17. `return -1;`
