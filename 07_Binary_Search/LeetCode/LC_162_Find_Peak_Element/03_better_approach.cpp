...
This particular file uses the "First falling edge" approach.
...
- `findPeakElement` is the function name the online judge calls.
...
----------------------------
Approach name: First falling edge
...
6. `for(int i = 0; i + 1 < nums.size(); i++) {`
   Starts a loop: first `int i = 0`; keep repeating while `i + 1 < nums.size()` is true; after each iteration perform `i++`.
7. `if(nums[i] > nums[i + 1]) return i;`
   Runs the next block only when `nums[i] > nums[i + 1]` is true. The one-line action is `return i;`.
8. `return nums.size() - 1;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "First falling edge" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "First falling edge" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
