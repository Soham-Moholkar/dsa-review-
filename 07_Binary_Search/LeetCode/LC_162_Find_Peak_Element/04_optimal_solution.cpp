...
This particular file uses the "Binary search on adjacent slope" approach.
...
----------------------------
Approach name: Binary search on adjacent slope
...
10. `if(nums[mid] < nums[mid + 1]) left = mid + 1;`
   Runs the next block only when `nums[mid] < nums[mid + 1]` is true. The one-line action is `left = mid + 1;`.
11. `else right = mid;`
   Handles the remaining case after the preceding condition(s) were false.
12. `return left;`
   Ends the function and sends `left` back to the caller.
