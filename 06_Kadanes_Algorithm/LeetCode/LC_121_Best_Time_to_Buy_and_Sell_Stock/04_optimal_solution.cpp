...
This particular file uses the "Minimum-so-far scan" approach.
...
----------------------------
Approach name: Minimum-so-far scan
...
7. `int maximumProfit = 0;`
   Creates `maximumProfit` and initializes it from `0`. This gives the algorithm its starting state.
...
9. `maximumProfit = max(maximumProfit, prices[i] - minimumPrice);`
   Updates `maximumProfit` to `max(maximumProfit, prices[i] - minimumPrice)` for the next step of the algorithm.
...
11. `return maximumProfit;`
   Ends the function and sends `maximumProfit` back to the caller.
