...
This particular file uses the "Bidirectional product scans" approach.
...
----------------------------
Approach name: Bidirectional product scans
...
11. `if(prefixProduct == 0) prefixProduct = 1;`
   Runs the next block only when `prefixProduct == 0` is true. The one-line action is `prefixProduct = 1;`.
12. `if(suffixProduct == 0) suffixProduct = 1;`
   Runs the next block only when `suffixProduct == 0` is true. The one-line action is `suffixProduct = 1;`.
13. `prefixProduct = prefixProduct * nums[i];`
   Updates `prefixProduct` to `prefixProduct * nums[i]` for the next step of the algorithm.
14. `suffixProduct = suffixProduct * nums[n - i - 1];`
   Updates `suffixProduct` to `suffixProduct * nums[n - i - 1]` for the next step of the algorithm.
15. `answer = max(answer, max(prefixProduct, suffixProduct));`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Bidirectional product scans" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Bidirectional product scans" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
