...
This particular file uses the "Kadane algorithm" approach.
...
----------------------------
Approach name: Kadane algorithm
...
6. `int currentSum = nums[0];`
   Creates `currentSum` and initializes it from `nums[0]`. This gives the algorithm its starting state.
7. `int maximumSum = nums[0];`
...
9. `currentSum = max(nums[i], currentSum + nums[i]);`
   Updates `currentSum` to `max(nums[i], currentSum + nums[i])` for the next step of the algorithm.
