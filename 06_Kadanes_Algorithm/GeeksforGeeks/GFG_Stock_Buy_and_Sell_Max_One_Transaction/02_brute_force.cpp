...
This particular file uses the "Try every buy and sell pair" approach.
...
----------------------------
Approach name: Try every buy and sell pair
...
7. `for(int buy = 0; buy < prices.size(); buy++) {`
   Starts a loop: first `int buy = 0`; keep repeating while `buy < prices.size()` is true; after each iteration perform `buy++`.
8. `for(int sell = buy + 1; sell < prices.size(); sell++) {`
   Starts a loop: first `int sell = buy + 1`; keep repeating while `sell < prices.size()` is true; after each iteration perform `sell++`.
9. `answer = max(answer, prices[sell] - prices[buy]);`
   Updates `answer` to `max(answer, prices[sell] - prices[buy])` for the next step of the algorithm.
10. `return answer;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Try every buy and sell pair" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the exhaustive "Try every buy and sell pair" strategy. The numbered walkthrough shows the complete candidate search performed by this implementation. Because every candidate allowed by the loops is examined before the answer is returned, a valid candidate cannot be skipped; the return/update condition keeps exactly the result required by the problem.
