...
This particular file uses the "Kadane state transition" approach.
...
----------------------------
Approach name: Kadane state transition
...
6. `long long currentSum = arr[0];`
   Creates `currentSum` and initializes it from `arr[0]`. This gives the algorithm its starting state.
7. `long long maximumSum = arr[0];`
...
9. `currentSum = max((long long)arr[i], currentSum + arr[i]);`
   Updates `currentSum` to `max((long long)arr[i], currentSum + arr[i])` for the next step of the algorithm.
10. `maximumSum = max(maximumSum, currentSum);`
   Updates `maximumSum` to `max(maximumSum, currentSum)` for the next step of the algorithm.
...
currentSum is the best sum ending at i; maximumSum is the best anywhere so far.
