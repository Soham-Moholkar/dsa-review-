...
This particular file uses the "Prefix and suffix product scans" approach.
...
----------------------------
Approach name: Prefix and suffix product scans
...
5. `int maxProduct(vector<int>& arr) {`
   Defines the judge-facing function and lists the inputs it receives.
6. `int prefixProduct = 1;`
   Creates `prefixProduct` and initializes it from `1`. This gives the algorithm its starting state.
7. `int suffixProduct = 1;`
   Creates `suffixProduct` and initializes it from `1`. This gives the algorithm its starting state.
8. `int answer = INT_MIN;`
...
11. `if(prefixProduct == 0) {`
   Runs the next block only when `prefixProduct == 0` is true.
12. `prefixProduct = 1;`
   Updates `prefixProduct` to `1` for the next step of the algorithm.
13. `if(suffixProduct == 0) {`
   Runs the next block only when `suffixProduct == 0` is true.
14. `suffixProduct = 1;`
   Updates `suffixProduct` to `1` for the next step of the algorithm.
15. `prefixProduct = prefixProduct * arr[i];`
   Updates `prefixProduct` to `prefixProduct * arr[i]` for the next step of the algorithm.
16. `suffixProduct = suffixProduct * arr[n - i - 1];`
   Updates `suffixProduct` to `suffixProduct * arr[n - i - 1]` for the next step of the algorithm.
17. `answer = max(answer, max(prefixProduct, suffixProduct));`
   Updates `answer` to `max(answer, max(prefixProduct, suffixProduct))` for the next step of the algorithm.
18. `return answer;`
...
5. DRY RUN
----------
Trace this exact file using the first example in `testcases.md`. It applies the "Prefix and suffix product scans" approach, so follow the numbered executable statements above and record each listed variable after it changes. Do not reuse the optimal implementation's saved variables: this file may enumerate candidates, sort values, or build auxiliary state instead.

When tracing by hand, write the important variables after every iteration. Do not jump directly to the final answer.

6. WHY THE ALGORITHM IS CORRECT
-------------------------------
This file uses the intermediate "Prefix and suffix product scans" strategy. Each operation in the numbered walkthrough preserves the information needed for the answer while arranging or storing it in a form that is easier to query. After every input element or required position has been processed, the final return/update condition selects the requested result.
