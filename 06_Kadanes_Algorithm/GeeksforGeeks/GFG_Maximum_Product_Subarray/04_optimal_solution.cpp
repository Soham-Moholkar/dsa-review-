...
This particular file uses the "Track maximum and minimum ending products" approach.
...
----------------------------
Approach name: Track maximum and minimum ending products
...
6. `int maximumEnding = arr[0];`
   Creates `maximumEnding` and initializes it from `arr[0]`. This gives the algorithm its starting state.
7. `int minimumEnding = arr[0];`
   Creates `minimumEnding` and initializes it from `arr[0]`. This gives the algorithm its starting state.
8. `int answer = arr[0];`
   Creates `answer` and initializes it from `arr[0]`. This gives the algorithm its starting state.
...
12. `maximumEnding = max(arr[i], maximumEnding * arr[i]);`
   Updates `maximumEnding` to `max(arr[i], maximumEnding * arr[i])` for the next step of the algorithm.
13. `minimumEnding = min(arr[i], minimumEnding * arr[i]);`
   Updates `minimumEnding` to `min(arr[i], minimumEnding * arr[i])` for the next step of the algorithm.
