...
6. `vector<long long> prefix(nums.size() + 1, 0);`
...
9. `long long maximumSum = LLONG_MIN;`
   Creates `maximumSum` and initializes it from `LLONG_MIN`. This gives the algorithm its starting state.
10. `for(int right = k; right <= nums.size(); right++) {`
   Starts a loop: first `int right = k`; keep repeating while `right <= nums.size()` is true; after each iteration perform `right++`.
11. `maximumSum = max(maximumSum, prefix[right] - prefix[right - k]);`
   Updates `maximumSum` to `max(maximumSum, prefix[right] - prefix[right - k])` for the next step of the algorithm.
12. `return (double)maximumSum / k;`
