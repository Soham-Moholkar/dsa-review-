...
This particular file uses the "Two-type sliding window" approach.
...
----------------------------
Approach name: Two-type sliding window
...
12. `int outgoing = fruits[left];`
   Creates `outgoing` and initializes it from `fruits[left]`. This gives the algorithm its starting state.
