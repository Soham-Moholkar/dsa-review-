...
8. `windowSum = windowSum + Arr[i];`
   Updates `windowSum` to `windowSum + Arr[i]` for the next step of the algorithm.
9. `long long answer = windowSum;`
   Creates `answer` and initializes it from `windowSum`. This gives the algorithm its starting state.
10. `for(int i = K; i < N; i++) {`
   Starts a loop: first `int i = K`; keep repeating while `i < N` is true; after each iteration perform `i++`.
11. `windowSum = windowSum + Arr[i];`
   Updates `windowSum` to `windowSum + Arr[i]` for the next step of the algorithm.
12. `windowSum = windowSum - Arr[i - K];`
   Updates `windowSum` to `windowSum - Arr[i - K]` for the next step of the algorithm.
13. `answer = max(answer, windowSum);`
   Updates `answer` to `max(answer, windowSum)` for the next step of the algorithm.
14. `return answer;`
