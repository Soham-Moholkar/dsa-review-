...
This particular file uses the "Sliding hashmap with zero-count erasure" approach.
...
----------------------------
Approach name: Sliding hashmap with zero-count erasure
...
7. `unordered_map<int, int> frequency;`
...
10. `answer.push_back(frequency.size());`
   Appends the computed value to the end of the result/container.
11. `for(int i = k; i < arr.size(); i++) {`
   Starts a loop: first `int i = k`; keep repeating while `i < arr.size()` is true; after each iteration perform `i++`.
12. `int outgoing = arr[i - k];`
   Creates `outgoing` and initializes it from `arr[i - k]`. This gives the algorithm its starting state.
13. `frequency[outgoing]--;`
   Moves the relevant counter or pointer by one position.
14. `if(frequency[outgoing] == 0) {`
   Runs the next block only when `frequency[outgoing] == 0` is true.
15. `frequency.erase(outgoing);`
   Removes the selected key/element so the container represents only currently relevant data.
16. `frequency[arr[i]]++;`
   Moves the relevant counter or pointer by one position.
17. `answer.push_back(frequency.size());`
   Appends the computed value to the end of the result/container.
18. `return answer;`
