...
This particular file uses the "Scan each window" approach.
...
----------------------------
Approach name: Scan each window
...
11. `value = A[i];`
   Updates `value` to `A[i]` for the next step of the algorithm.
12. `break;`
   Stops the nearest loop immediately because no more iterations are needed.
13. `answer.push_back(value);`
