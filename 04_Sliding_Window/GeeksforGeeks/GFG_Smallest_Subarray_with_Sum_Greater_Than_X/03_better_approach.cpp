...
This particular file uses the "Prefix sums with binary search" approach.
...
----------------------------
Approach name: Prefix sums with binary search
...
5. `int smallestSubWithSum(int x, vector<int>& arr) {`
...
12. `long long required = prefix[left] + x + 1;`
   Creates `required` and initializes it from `prefix[left] + x + 1`. This gives the algorithm its starting state.
13. `int rightPrefix = lower_bound(prefix.begin() + left + 1, prefix.end(), required) - prefix.begin();`
   Creates `rightPrefix` and initializes it from `lower_bound(prefix.begin() + left + 1, prefix.end(), required) - prefix.begin()`. This gives the algorithm its starting state.
14. `if(rightPrefix <= n) {`
   Runs the next block only when `rightPrefix <= n` is true.
15. `answer = min(answer, rightPrefix - left);`
   Updates `answer` to `min(answer, rightPrefix - left)` for the next step of the algorithm.
