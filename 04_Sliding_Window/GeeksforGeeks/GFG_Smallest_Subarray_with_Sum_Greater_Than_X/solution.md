# Smallest Subarray with Sum Greater Than X — explained solution
...
- **Brute Force:** Try every start and end. Time O(n^2); space O(1).
- **Better:** Prefix sums with binary search. Time O(n log n); space O(n).
...
```bash
python3 scripts/test_solutions.py --problem GFG_Smallest_Subarray_with_Sum_Greater_Than_X --sanitize
```
